...
export function SourcePage() {
  ...
  const sourceName = SOURCE_SLUGS[slug] ?? slug;
  ...
  useEffect(() => {
    ...
    Promise.all([fetchArchiveMeta(), fetchSearchIndex()])
      .then(([meta, idx]) => {
        const filtered = idx.items.filter((i) => i.src === slug);
        setItems(filtered);
        const src = meta.sources.find((s) => s.slug === slug);
        setCount(src?.count ?? filtered.length);
      })
  ...
  return (
    <div className="space-y-6">
      <section>
        <p className="ss-section-label mb-2">Source</p>
        <h1 className="mb-2 text-2xl font-bold tracking-tight text-text-primary">{sourceName}</h1>
      ...
      {pageItems.length === 0 ? (
        <p className="text-text-tertiary">이 소스의 아카이브 항목이 없습니다.</p>
      ) : (
